...
function AutoGearFindTemporaryEmptyBagSlot(excludedContainer)
	...
	do
		...
		if (bagType == 0 and freeSlots and freeSlots > 0) then
			return BACKPACK_CONTAINER
		end
	end

	-- normal bags 1 ~ NUM_BAG_SLOTS, excluding the bag being replaced
...
function AutoGearTryMoveBagItemToTemporarySlot(sourceContainer, sourceSlot, excludedContainer)
	local tempContainer = AutoGearFindTemporaryEmptyBagSlot(excludedContainer)
	...
	if tempContainer == BACKPACK_CONTAINER then
		PutItemInBackpack()
	else
		PutItemInBag(CONTAINER_BAG_OFFSET + tempContainer)
	end
					...
					if (GetTime() > curAction.t) then
						if ((AutoGearDB.Enabled ~= nil) and (AutoGearDB.Enabled == true)) then
							...
							elseif (curAction.ensuringEquipped) then
								...
								-- if the replacement bag item is inside the bag we are trying to replace,
								-- do not try temporary manual moves for bag items; use Auto Sort and retry.
